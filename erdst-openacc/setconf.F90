...
! setparam(module setconf内)のOpenACCデータ管理ディレクティブをOpenMP
! targetへ変換した検証用ブランチのバージョンです。計算ロジックは不変。
! ここで確保・転送している moltype, numsite, sluvid, charge, ljtype,
! mol_begin_index, mol_charge, ljlensq_mat, ljene_mat は engmain 側の
! データで、realcal.F90 / recpcal.F90 のカーネルから present/map(alloc:)
! で参照されています。今回これらの確保・転送そのものをOpenMPに変換した
! ことで、realcal.F90 / recpcal.F90 側が今まで頼っていた「OpenACCと
! OpenMP targetのpresence table共有」という非公式挙動への依存を、この
! データについては解消しています(=正規のOpenMP targetのpresence
! tableだけで完結する)。
! なお本ファイルには計算カーネル(!$acc parallel loop等)は元々存在せず、
! enter data / update のみなので、変換は機械的です。
! =====================================================================
!
